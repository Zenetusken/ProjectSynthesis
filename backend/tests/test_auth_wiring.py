...
async def test_optimize_sets_user_id_from_jwt():
    ...
    Implementation note: optimize_prompt returns a StreamingResponse immediately
    after calling session.add(optimization) + session.commit(). The event_stream
    generator (which runs the pipeline) is only iterated by the ASGI server —
    not by this test. We capture the Optimization object at session.add() time,
    which is where user_id is set (at object construction, before the response).
    """
    ...
    mock_session.add = MagicMock(side_effect=_capture)  # add() is sync in SQLAlchemy
    ...
    # fetch_url_contexts is awaited before the StreamingResponse is returned —
    # patch it so the function doesn't attempt a real network call.
    with patch("app.routers.optimize.fetch_url_contexts", AsyncMock(return_value=[])):
        response = await optimize_prompt(
            optimize_req, mock_req, mock_session,
            current_user=current_user,
        )

    # session.add() must have been called before the StreamingResponse was returned.
